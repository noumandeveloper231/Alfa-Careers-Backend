...
export const sendPhoneVerificationOtp = async (req, res) => {
    ...
    try {
        ...
        user.phoneVerificationOtpExpires = Date.now() + 10 * 60 * 1000; // 10 min expiration
        await user.save();

        // Send OTP via WhatsApp
        const response = await fetch(`${selfUrl}/api/whatsapp/send-message`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                messageBody: `Here is your OTP for <b>${phoneNumber}</b> verification: <b>${otp}</b>`,
...
export const verifyPhone = async (req, res) => {
    ...
    try {
        ...
        const isMatched = await bcrypt.compare(otp, user.phoneVerificationOtp);
        ...
        fetch(`${selfUrl}/api/whatsapp/send-message`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                messageBody: `*Congratulations!* Your phone number *${phoneNumber}* has been *verified* successfully.\n\nThank you for using *Alfa Career*.`,
                phoneNumber
            })
        }).catch(err => {
